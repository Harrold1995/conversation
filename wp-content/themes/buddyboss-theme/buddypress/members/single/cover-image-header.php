...
$user_has_cover           = function_exists( 'bp_attachments_get_user_has_cover_image' ) ? bp_attachments_get_user_has_cover_image( $displayed_user->id ) : true;
$has_default_cover        = function_exists( 'bb_attachment_get_cover_image_class' ) ? bb_attachment_get_cover_image_class( $displayed_user->id, 'user' ) : '';
?>
	...
	<div id="cover-image-container">
		...
		<div id="header-cover-image" class="<?php echo esc_attr( 'cover-' . $profile_cover_height . ' width-' . $profile_cover_width . $has_cover_image_position . $has_cover_image . $has_default_cover ); ?>">
			...
			<?php if ( bp_is_my_profile() ) { ?>
				<a href="<?php echo bp_get_members_component_link( 'profile', 'change-cover-image' ); ?>" class="link-change-cover-image" data-balloon-pos="right" data-balloon="<?php esc_attr_e( 'Change Cover Photo', 'buddyboss-theme' ); ?>">
					<i class="bb-icon-edit-thin"></i>
				</a>

				<?php if ( ! empty( $cover_image_url ) && $user_has_cover ) { ?>
					...
					<div class="header-cover-reposition-wrap">
						...
						<?php if ( ! empty( $cover_image_url ) ) { ?>
							<img src="<?php echo esc_url( $cover_image_url ); ?>" alt="<?php esc_attr_e( 'Cover photo', 'buddyboss-theme' ); ?>" />
		...
		<div id="item-header-cover-image" class="item-header-wrap <?php echo $class; ?>">
			...
			<div id="item-header-avatar">
				<?php if ( bp_is_my_profile() && ! bp_disable_avatar_uploads() ) { ?>
					<a href="<?php bp_members_component_link( 'profile', 'change-avatar' ); ?>" class="link-change-profile-image" data-balloon-pos="down" data-balloon="<?php esc_attr_e( 'Change Profile Photo', 'buddyboss-theme' ); ?>">
						<i class="bb-icon-edit-thin"></i>
					</a>
				<?php } ?>
