...
= 2.1.1 =
* Tweak: Added a filter to change login url in reviews section. 
* Fix: Added double space in Primary Key for table creation.
* Fix: Added fix where the_content was being emptied in an errant condition.
* Fix: Removed flush_rewrite_rules on each init call.
* Fix: Fixed Course Survey dropdown not showing selected value issue.
* Fix: Fixed modal class conflict with BuddyBoss theme when using Analytics code issue.
* Fix: Changed default dropdown value for reviews and star rating display in course edit to on completion.
* Fix: Fixed survey button shown after deletion.
* Fix: Added CSS patch for ratings alignment.
* Fix: Fixed Reminder email spamming issue.
* Fix: Added fix for prompt email not sending in certain conditions.
* Fix: Added fixes to remove PHP Notices.

= 2.1.0 =
* Feature: Ability to upload images and videos for reviews.
* Feature: Ability to upload images for feedback.
* Feature: Introducing Course Surveys where you will be able to ask pointed questions regarding student experiences.
* Feature: Added a feature which automatically triggers the submit review modal after course completion.
* Feature: Added a global setting to enable reviews for all courses.(Note: Course specific setting takes precedence over global setting).
* Feature: Added a new shortcode to show all course reviews on a single page.
* Feature: Added feature to send reminder emails to students to post reviews.
* Fix: Fixed JS error on feedback submission.
