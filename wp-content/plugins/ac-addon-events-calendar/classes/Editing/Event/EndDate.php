...
class EndDate implements ACP\Editing\Service {

	public function get_value( $id ) {
		return get_post_meta( $id, '_EventEndDate', true );
	}

	public function update( Request $request ) {
		$id = $request->get( 'id' );
		$start_date = DateTime::createFromFormat( 'Y-m-d H:i:s', $request->get( 'value' ) );
		$end_date = DateTime::createFromFormat( 'Y-m-d H:i:s', get_post_meta( $id, '_EventStartDate', true ) );
