...
function badgeos_learndash_trigger_award_points_event() {
    ...
    $learndash_assignment_approve_triggers = array(
    ...
    if ( in_array($this_trigger, $learndash_assignment_approve_triggers ) ) {
...
function badgeos_learndash_trigger_event() {
    ...
    if ( in_array($this_trigger, $learndash_assignment_approve_triggers ) ) {
        $assignment_id = $args[0];
        $userID = get_post_meta( $assignment_id, 'user_id', true );
    }
...
function badgeos_learndash_award_quiz_points_as_badge_points( $quizdata, $user ) {
     ...
     */
    $user_id 		= $user->ID;
    $total_points 	= absint( $quizdata['points'] );
    ...
    $count = $wpdb->get_var(
        $wpdb->prepare( 
            "SELECT COUNT(*) AS count FROM {$wpdb->prefix}badgeos_points 
            WHERE credit_id = %d AND user_id = %d AND this_trigger = %s;", $points_type, $user_id, 'bos_ld_quiz_based' )
    );


    /**
     * if option is on then check if points already awarded
     */
    if( $wn_bos_ld_options['badgeos_learndash_quiz_multi_time_point_award'] == 1 ){
         if( $count > 0 ) {
            return;
        }
    }
...
add_action( 'learndash_quiz_submitted', 'badgeos_learndash_award_quiz_points_as_badge_points', 15, 2 );
...
function badgeos_learndash_user_deserves_learndash_step( $return, $user_id, $achievement_id, $this_trigger = '', $site_id = 1, $args = array() ) {
    ...
    if ( 'learndash_trigger' == $requirements[ 'trigger_type' ] ) {
        ...
        if ( isset( $requirements['learndash_object_arg1'] ) )
                    $object_arg2 = $requirements['learndash_object_arg2'];
        /**
        ...
        /**
         * Assignment-upload triggers
         */
        $learndash_assignment_upload_triggers = array(
        ...
        if ( is_array( $arg_data ) ) {
            if ( isset( $arg_data[ 'quiz' ] ) ) {
                if ( $arg_data['quiz'] != null ){
                    $triggered_object_id = (int) $arg_data[ 'quiz' ];
                } else {
         ...
         * Assignment upload triggers
         */        
        if ( in_array( $learndash_trigger, $learndash_assignment_upload_triggers ) ) {

            $assignment_data = $args[1];
            $assignment_lesson_id = $assignment_data['lesson_id'];
            $assignment_course_id = $assignment_data['course_id'];

            if ( 'badgeos_learndash_upload_an_assignment' == $learndash_trigger ) {
                $learndash_triggered = true;
            } elseif ( 'badgeos_learndash_upload_an_assignment_specific_lesson' == $learndash_trigger ) {
                if ( $object_id == $assignment_lesson_id ) {
                    // check if assignment is uploaded to specific lesson
                    $learndash_triggered = true;
                }
            } elseif ( 'badgeos_learndash_upload_an_assignment_specific_course' == $learndash_trigger ) {
                if ( $object_id == $assignment_course_id ) {
                    // check if assignment is uploaded to specifc course
                    $learndash_triggered = true;
                }
            }
        }
         ...
         * Assignment approve triggers
         */        
        if ( in_array( $learndash_trigger, $learndash_assignment_approve_triggers ) ) {
            $assignment_id = $args[0];
            $assignment_lesson_id = get_post_meta($assignment_id, 'lesson_id', true);
            $assignment_course_id = get_post_meta($assignment_id, 'course_id', true);
            if ( 'badgeos_learndash_approve_an_assignment' == $learndash_trigger ) {
                $learndash_triggered = true;
            } elseif ( 'badgeos_learndash_approve_an_assignment_specific_lesson' == $learndash_trigger ) {
                if ( $object_id == $assignment_lesson_id ) {
                    // check if assignment is uploaded to specific lesson
                    $learndash_triggered = true;
                }
            } elseif ( 'badgeos_learndash_approve_an_assignment_specific_course' == $learndash_trigger ) {
                if ( $object_id == $assignment_course_id ) {
                    // check if assignment is uploaded to specifc course
                    $learndash_triggered = true;
                }            
         ...
         * Quiz triggers
         */
        if ( in_array( $learndash_trigger, $learndash_quiz_triggers ) ) {
            ...
            } elseif('badgeos_learndash_quiz_completed_percentage' == $learndash_trigger) {
                $percentage = (int) $arg_data[ 'percentage' ];
                $object_arg1 = (int) $object_arg1;
                $object_arg2 = (int) $object_arg2;

                if ( $percentage >= $object_arg1 && $percentage <= $object_arg2) {
                    $learndash_triggered = true;
                } else {
                    $learndash_triggered = false;
                }
...
function badgeos_learndash_user_deserves_credit_deduct( $return, $credit_step_id, $credit_parent_id, $user_id, $this_trigger, $site_id, $args ) {
    ...
    if ( 'learndash_trigger' == $requirements[ 'trigger_type' ] ) {
        ...
        if ( isset( $requirements['learndash_object_arg2'] ) )
            $object_arg2 = $requirements['learndash_object_arg2'];
...
// Quiz fail triggers fix
function learndash_quiz_submitted_cb( $quizdata, $user ) {

    do_action( 'badgeos_learndash_quiz_completed_fail', $quizdata, $user );
    do_action( 'badgeos_learndash_quiz_completed_fail_tag', $quizdata, $user );
}
add_action( 'learndash_quiz_submitted', 'learndash_quiz_submitted_cb', 20, 2 );