...
function badgeos_learndash_step_requirements( $requirements, $step_id ) {
    ...
    $requirements[ 'learndash_object_arg2' ] = (int) get_post_meta( $step_id, '_badgeos_learndash_object_arg2', true );
...
function badgeos_learndash_step_etc_select( $step_id, $post_id ) {
    ...
    $current_object_arg2 = (int) get_post_meta( $step_id, '_badgeos_learndash_object_arg2', true );
    ...
    $min_grade = 0;
    $max_grade = 100;
    if ( in_array( $current_trigger, array( 'badgeos_learndash_quiz_completed_percentage' ) ) ) {
        $min_grade = (int) $current_object_arg1;
        $max_grade = (int) $current_object_arg2;
    
    }
    echo '<span><br><br>Award for:  Min: <input name="badgeos_learndash_quiz_min_grade" class="input-quiz-min-grade" type="text" value="' . $min_grade . '" size="3" maxlength="3" placeholder="0" />%</span>';    

    echo '<span style="margin-left:7px">To Max: <input name="badgeos_learndash_quiz_max_grade" class="input-quiz-max-grade" type="text" value="' . $max_grade . '" size="3" maxlength="3" placeholder="100" />%</span>';
    ...
    if ( !empty( $objects ) ) {
        foreach ( $objects as $object ) {
            ...
            if ( in_array( $current_trigger, array( 'learndash_lesson_completed', 'badgeos_learndash_upload_an_assignment_specific_lesson', 'badgeos_learndash_approve_an_assignment_specific_lesson' ) ) )
                $selected = selected( $current_object_id, $object->ID, false );
    ...
    if ( !empty( $objects ) ) {
        foreach ( $objects as $object ) {
            ...
            if ( in_array( $current_trigger, array( 'learndash_course_completed', 'badgeos_learndash_upload_an_assignment_specific_course', 'badgeos_learndash_approve_an_assignment_specific_course' ) ) )
                $selected = selected( $current_object_id, $object->ID, false );
    ...
    if( isset( $ld_settings ) && $ld_settings['wp_post_tag'] == 'yes' ) {
...
function badgeos_learndash_save_step( $title, $step_id, $step_data ) {
    ...
    if ( 'learndash_trigger' == $step_data[ 'trigger_type' ] ) {
        ...
        if ( 'learndash_quiz_completed' == $step_data[ 'learndash_trigger' ] ) {
        ...
        } elseif ( 'badgeos_learndash_quiz_completed_percentage' == $step_data[ 'learndash_trigger' ] ) {

            /**
             * Get Object ID
             */
            $object_id = (int) $step_data[ 'learndash_quiz_id' ];
            $object_arg1 = (int) $step_data[ 'learndash_quiz_min_grade' ];
            $object_arg2 = (int) $step_data[ 'learndash_quiz_max_grade' ];

            /**
             * Set new step title
             */
            if ( empty( $object_id ) ) {
                $title = sprintf( __( 'Completed quiz with percentage between %d and %d', 'badgeos-learndash' ), $object_arg1, $object_arg2 );
            } else {
                $title = sprintf( __( 'Completed quiz %s with percentage between %d and %d', 'badgeos-learndash' ), get_the_title( $object_id ), $object_arg1, $object_arg2 );
            }
        ...
        } elseif ( 'badgeos_learndash_upload_an_assignment_specific_lesson' == $step_data[ 'learndash_trigger' ] ) {
             ...
             */
            if ( empty( $object_id ) ) {
                $title = __( 'Upload assignment of specific lesson', 'badgeos-learndash' );
            } else {
                $title = sprintf( __( 'Upload assignment of specific lesson "%s"', 'badgeos-learndash' ), get_the_title( $object_id ) );
            }
        } elseif ( 'badgeos_learndash_approve_an_assignment_specific_lesson' == $step_data[ 'learndash_trigger' ] ) {
             ...
             */
            if ( empty( $object_id ) ) {
                $title = __( 'Approve an assignment of specific lesson', 'badgeos-learndash' );
            } else {
                $title = sprintf( __( 'Approve an assignment of specific lesson "%s"', 'badgeos-learndash' ), get_the_title( $object_id ) );
            }
        ...
        } elseif ( 'badgeos_learndash_upload_an_assignment_specific_course' == $step_data[ 'learndash_trigger' ] ) {
             ...
             */
            if ( empty( $object_id ) ) {
                $title = __( 'Upload an assignment of specific course', 'badgeos-learndash' );
            }  else {
                $title = sprintf( __( 'Upload an assignment of specific course "%s"', 'badgeos-learndash' ), get_the_title( $object_id ) );
            }
        } elseif ( 'badgeos_learndash_approve_an_assignment_specific_course' == $step_data[ 'learndash_trigger' ] ) {
             ...
             */
            if ( empty( $object_id ) ) {
                $title = __( 'Approve an assignment of specific course', 'badgeos-learndash' );
            }  else {
                $title = sprintf( __( 'Approve an assignment of specific course "%s"', 'badgeos-learndash' ), get_the_title( $object_id ) );
            }
        ...
        update_post_meta( $step_id, '_badgeos_learndash_object_arg2', $object_arg2 );
    }

    return $title;
}
add_filter( 'badgeos_save_step', 'badgeos_learndash_save_step', 15, 3 );
...
function badgeos_learndash_step_js() {
    ...
    <script type="text/javascript">
        jQuery( document ).ready( function ( $ ) {
            ...
            // This fix will be removed once fixed in badgeos
            $('.step-title .title').each( function(){
                var step_id = '', temp = '';
                temp = $(this).attr('id');
                temp = temp.split('-');
                step_id = temp[1];
                $.ajax({
                    ...
                    data: {
                        'action': 'return_step_title',
                        'step_id': step_id
                    },
                    success: function( response ) {
                        var title = response.data.step_title;
                        title = title.replace('&#8220;', '"');
                        title = title.replace('&#8221;', '"');
                        $('#step-'+step_id+'-title').val( title );
                    }
                });
            });
            ...
            $( document ).on( 'change', '.select-trigger-type', function () {
                ...
                var times = trigger_parent.find('.required-count').val();
                /**
                ...
                if ( 'learndash_trigger' == trigger_type.val() ) {
                    ...
                    if ( 'badgeos_learndash_quiz_completed_percentage'  == trigger ) {
                        trigger_parent.find('.input-quiz-min-grade').parent().show();
                        trigger_parent.find('.input-quiz-max-grade').parent().show();
                    }
                    if( parseInt( times ) < 1 )
                    ...
                    trigger_parent.find( '.input-quiz-min-grade' ).parent().hide();
                    trigger_parent.find( '.input-quiz-max-grade' ).parent().hide();
            ...
            $( document ).on( 'update_step_data', function ( event, step_details, step ) {
                ...
                step_details.learndash_quiz_min_grade = $( '.input-quiz-min-grade', step ).val();
                step_details.learndash_quiz_max_grade = $( '.input-quiz-max-grade', step ).val();
        ...
        function badgeos_learndash_step_change( $this , times) {
            ...
            times = trigger_parent.find('.required-count').val();
            ...
            trigger_parent.find( '.select-quiz-id' )
                .toggle(
                    ( 'learndash_quiz_completed' == trigger_value
                        ...
                        || 'badgeos_learndash_quiz_completed_fail' == trigger_value || 'badgeos_learndash_quiz_completed_percentage' == trigger_value )
                );
            ...
            trigger_parent.find( '.select-lesson-id' )
                .toggle( 'learndash_lesson_completed' == trigger_value 
                    || 'badgeos_learndash_upload_an_assignment_specific_lesson' == trigger_value 
                    || 'badgeos_learndash_approve_an_assignment_specific_lesson' == trigger_value );
            ...
            trigger_parent.find( '.select-course-id' )
                .toggle( 'learndash_course_completed' == trigger_value 
                    || 'badgeos_learndash_upload_an_assignment_specific_course' == trigger_value 
                    || 'badgeos_learndash_approve_an_assignment_specific_course' == trigger_value );
            ...
            /**
             * Quiz Result Stratification
             */    
            trigger_parent.find( '.input-quiz-min-grade' ).parent() // target parent span
                .toggle( 'badgeos_learndash_quiz_completed_percentage' == trigger_value );
            trigger_parent.find( '.input-quiz-max-grade' ).parent() // target parent span
                .toggle( 'badgeos_learndash_quiz_completed_percentage' == trigger_value ); 
            /**
             * Group
             */
            ...
            if ( ( 'learndash_quiz_completed' == trigger_value && '' != trigger_parent.find( '.select-quiz-id' ).val() )
                ...
                || ( 'badgeos_learndash_quiz_completed_percentage' == trigger_value && '' != trigger_parent.find( '.select-quiz-id' ).val() )
                ...
                || ( 'badgeos_learndash_upload_an_assignment_specific_lesson' == trigger_value && '' != trigger_parent.find( '.select-lesson-id' ).val() )
                || ( 'badgeos_learndash_approve_an_assignment_specific_lesson' == trigger_value && '' != trigger_parent.find( '.select-lesson-id' ).val() )
                || ( 'learndash_course_completed' == trigger_value && '' != trigger_parent.find( '.select-course-id' ).val() )
                || ( 'badgeos_learndash_upload_an_assignment_specific_course' == trigger_value && '' != trigger_parent.find( '.select-course-id' ).val() )
                || ( 'badgeos_learndash_approve_an_assignment_specific_course' == trigger_value && '' != trigger_parent.find( '.select-course-id' ).val() )
