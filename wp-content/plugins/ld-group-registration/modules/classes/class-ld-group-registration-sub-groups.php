...
if ( ! class_exists( 'Ld_Group_Registration_Sub_Groups' ) ) {
	...
	class Ld_Group_Registration_Sub_Groups {
		...
		private function sub_group_update( $edit = false ) {
			...
			if ( $edit ) {
				$sub_group_id   = $_POST['subGroupId'];
				$sub_group_edit = array(
					'ID'         => $sub_group_id,
				...
				$sub_group_data = array(
					...
					'post_status'  => 'publish',
					'post_type'    => 'groups',
					'post_parent'  => $_POST['parentGroupId'],
					'post_content' => '',
					'post_author'  => get_current_user_id(),
					'post_name'    => sanitize_title( $_POST['groupName'] ),
				);
			...
			if ( $should_update ) {
				...
				update_post_meta( $sub_group_id, 'parent_group_id', $_POST['parentGroupId'] );
				$parent_sub_group_ids     = ldgr_get_sub_group_ids( $_POST['parentGroupId'] );
				$new_parent_sub_group_ids = ( $parent_sub_group_ids && count( $parent_sub_group_ids ) ) ? $parent_sub_group_ids : array();
				array_push( $new_parent_sub_group_ids, $sub_group_id );
				update_post_meta( $_POST['parentGroupId'], 'sub_groups', serialize( $new_parent_sub_group_ids ) );

				// Parent seat limit - ( submitted in the form - existing seat limit of the sub-group ). This is to prevent false reduction of the parent group sear limit.
				$final_parent_group_limit = (int) $_POST['parentGroupLimit'] - ( (int) $_POST['groupSeats'] - $existing_sg_limit );

				// updating the parent Group's user seats balance.
				update_post_meta( $_POST['parentGroupId'], 'wdm_group_users_limit_' . $_POST['parentGroupId'], $final_parent_group_limit );

				foreach ( $_POST['groupCourses'] as $course ) {
					update_post_meta( $course, 'learndash_group_enrolled_' . $sub_group_id, time() );
				}

				// Let's make all selected leaders as group leaders.
				if ( ! empty( $_POST['groupLeaders'] ) ) {
					foreach ( $_POST['groupLeaders'] as $sub_group_leader_id ) {
						$this->change_user_role_to_gl( (int)$sub_group_leader_id );
					}	
				}

				learndash_set_groups_administrators( $sub_group_id, $_POST['groupLeaders'] );

				foreach ($_POST['groupUsers'] as $group_user ) {
					ld_update_group_access( $group_user, $sub_group_id );
				}

				update_post_meta( $sub_group_id, 'wdm_group_users_limit_' . $sub_group_id, intval( $_POST['groupSeats'] ) );
		...
		private function change_user_role_to_gl( $user_id ) {
			...
			if ( ! empty( $user_id ) ) {
				 ...
				 */
				$dont_remove_user_roles = apply_filters( 'ldgr_sub_group_leaders_remove_prev_role', array(
						'administrator',
						'wdm_instructor'
					)
				);
		...
		public function add_custom_sub_group_label_setting( $custom_label_setting ) {
			...
			$custom_label_setting['subgroup'] = array(
				'name'      => 'subgroup',
				'type'      => 'text',
			...
			$custom_label_setting['subgroups'] = array(
				'name'      => 'subgroups',
				'type'      => 'text',
			...
			if ( $is_heirarchical && ! $parent_id ) {
				...
				array_unshift(
					...
					array(
						'id'       => $tab_id,
						'active'   => true,
						'template' => plugin_dir_path( dirname( __FILE__ ) ) . 'templates/ldgr-group-users/tabs/sub-groups-tab.template.php',
					)
				);
			}
			return $tab_contents;
