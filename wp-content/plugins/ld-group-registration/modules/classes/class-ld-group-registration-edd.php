		...
		public function add_edd_cart_item_name( $download_title, $download_id, $item ) {
			...
			if ( 'on' == $value ) {
				if ( isset( $item['options']['wdm_ld_group_active'] ) ) {
					$temp = $download_title . ' ' . apply_filters( 'wdm_group_registration_label_below_product_name', __( 'Group Registration', 'wdm_ld_group' ), $download_title, $download_id, $item );
					if ( array_key_exists( 'ldgr_group_name', $item['options'] ) && ! empty( $item['options']['ldgr_group_name'] ) ) {
						$temp = $download_title . __( ' - ', 'wdm_ld_group' ) . $item['options']['ldgr_group_name'];
					}
					return $temp;
				}
			}
				...
				if ( 'on' == $value_show ) {
					...
					<div class="wdm_group_registration">
						...
						<label for="wdm_gr_signle"> 
							<?php echo esc_html( apply_filters( 'wdm_gr_single_label', __( 'Individual', 'wdm_ld_group' ) ) ); ?>
						</label>
						<input
							type="radio"
							name="wdm_ld_group_active"
						...
						<label for="wdm_gr_group"> 
							<?php echo esc_html( apply_filters( 'wdm_gr_group_label', __( 'Group', 'wdm_ld_group' ) ) ); ?>
						</label>
					</div>
					<?php
				}
							...
							<?php
								echo esc_html(
									apply_filters(
										...
										__( 'This will add Group Leader as Group Member & will charge for it.', 'wdm_ld_group' )
									)
								);
							?>
						</span>
		...
		public function add_additional_group_details( $html, $download_id = 0, $args = array() ) {
			if ( edd_item_quantities_enabled() && ! edd_download_quantities_disabled( $download_id ) ) {
				...
				<div class="ldgr_group_name" <?php echo ( 'individual' == $default_option ) ? 'style="display:none;"' : ''; ?>>
					<label for="ldgr_group_name"><?php esc_html_e( 'Group Name', 'wdm_ld_group' ); ?></label>
					<input type="text" name="ldgr_group_name" value="<?php echo $group_name; ?>" placeholder="<?php _e( 'Enter a name for your group', 'wdm_ld_group' ); ?>" <?php echo empty( $group_name ) ? '' : 'readonly'; ?>/>
				</div>
				<?php
				$html .= ob_get_clean();
			}
