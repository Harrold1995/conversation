jQuery(document).on('ready', function(){
    ...
    jQuery('.edit-sub-group-submit').on('click', function(e){
        e.preventDefault();
        var groupLeaders = jQuery('.edit-sub-group-leader:checkbox:checked').map(function() {
            return this.value;
        }).get();

        var groupUsers = jQuery('.edit-sub-group-user:checkbox:checked').map(function() {
            return this.value;
        }).get();

        var groupName         = jQuery('.edit-sub-group-name').val();
        var groupSeats        = jQuery('.edit-sub-group-seat').val();
        ...
        var prevGroupSeats    = jQuery('input[name="previous-edit-sub-group-seat"]').val();

        if ( ( parseInt(groupSeats) - parseInt( prevGroupSeats ) ) > parentGroupLimit) {
            alert(ldgr_loc.group_limit);
        } else {
            jQuery.ajax({
                url: ldgr_loc.ajax_url,
        ...
        jQuery.ajax({
            ...
            success : function(response){
                if ('error' == response.status) {
                    ...
                    jQuery('.edit-sub-group-seat').val(response.sub_group_limit);
                    jQuery('input[name="previous-edit-sub-group-seat"]').val(response.sub_group_limit);
                    jQuery('.edit-sub-group-leader:checkbox').each(function() {
                        if(jQuery.inArray(parseInt(jQuery(this).val()), response.sub_group_leaders) != -1) {
                            jQuery(this).prop('checked', true);
                        }
                    });
                    jQuery('.edit-sub-group-user:checkbox').each(function() {
                        if(jQuery.inArray(parseInt(jQuery(this).val()), response.sub_group_users) != -1) {
                            jQuery(this).prop('checked', true);
                        }
                    });
                    var selectedCourses = []
                    jQuery('.edit-sub-group-courses > option').each(function() {
                        if(jQuery.inArray(parseInt(jQuery(this).val()), response.sub_group_courses) != -1) {
