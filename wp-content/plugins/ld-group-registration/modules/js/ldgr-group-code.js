jQuery(document).ready(function(){
    ...
    jQuery('.ldgr-validate-group-code .empty-bg').on( 'checkboxToggle', function(){
        ...
        jQuery.ajax({
            ...
            success: function( response ) {
                ...
                if ( 'error' == response.type) {
                    ...
                    // }
                    if ( jQuery('.ldgr-group-code-items .ldgr-group-code-item').length ) {
                        jQuery('.ldgr-group-code-items .ldgr-group-code-item').first().before( response.row_html );
                    } else {
                        jQuery( '.ldgr-group-code-items .ldgr-no-group-codes' ).remove();
                        jQuery('.ldgr-group-code-items').append( response.row_html );
                    }
                    jQuery('#ldgr-group-code-create-form').find('.gcs-cancel').trigger('click');
                }
    ...
    jQuery('.ldgr-group-code-items').on( 'checkboxToggle', '.ldgr-gr-code-status-wrap .ldgr-toggle-wrap .empty-bg', function() {
        var checked = false;
        if ( jQuery(this).parent().hasClass('enabled') ) {
            checked = true;
        }
    ...
    // Group code edit
    jQuery('.ldgr-group-code-items').on('click', '.ldgr-edit-code', function(){
        // Reset form data
            ...
            success: function( response ) {
                if ('error' == response.type) {
                    ...
                    if ('on' == response.data.validation_check) {
                        $group_code_edit_form.find('.ldgr-code-validation-check').val('on');
                        $group_code_edit_form.find('.ldgr-validate-group-code').addClass('enabled');
                        ...
                        $group_code_edit_form.find('.ldgr-validation-rules').slideDown();
                    }

                    if (response.data.status) {
                        $group_code_edit_form.find('.ldgr-code-status').val('on');
                    }
        ...
        jQuery.ajax({
            ...
            success: function( response ) {
                ...
                if ( 'error' == response.type) {
                    ...
                    jQuery('.ldgr-group-code-items').find('#' + response.row_id).remove();
                    jQuery('.ldgr-group-code-items .ldgr-group-code-item').first().before( response.row_html );
                    ...
                    setTimeout(() => {
                        jQuery('#ldgr-group-code-edit-form').find('.gcs-cancel').trigger('click');
                    }, 4000);
                }
                jQuery('html, body').animate({
                    scrollTop: jQuery('.ldgr-group-code-tab').offset().top - 20
    ...
    // Group Code Delete
    jQuery('.ldgr-group-code-items').on('click', '.ldgr-delete-code', function() {
            ...
            jQuery.ajax({
                ...
                success: function( response ) {
                    ...
                    if ( 'error' == response.type) {
                        ...
                        jQuery('.ldgr-group-code-messages' ).removeClass( 'error' ).addClass( 'success' ).show();
                        jQuery('div.ldgr-group-code-items '+ response.row_id).fadeOut('slow', function() {
                            jQuery(this).remove();
                        });
                    }
                    jQuery('html, body').animate({
    ...
    jQuery('.ldgr-group-code-items').on('click', '.ldgr-cp-code', function(){
        var group_code_id = jQuery(this).data('id');
        var group_code = jQuery('#ldgr-group-code-row-'+group_code_id+' .ldgr-group-code-info .ldgr-group-code');
