...
function ldgr_get_leader_group_ids( $user_id = 0 ) {
	...
	// If group hierarchy enabled, don't show sub-grops separately.
	if ( learndash_is_groups_hierarchical_enabled() ) {
		$sub_groups_ids = array();
...
function ldgr_get_sub_group_ids( $group_id = 0 ) {
	$sub_group_ids = array();
	if ( ! empty( $group_id ) ) {
		$sub_group_ids = unserialize( get_post_meta( $group_id, 'sub_groups', true ) );
	}
...
function ldgr_get_groups_dashboard_page() {
	...
	$groups_dashboard_pages = $groups_dashboard_pages[0];

	/**
	 * Filter get dashboard page list.
	 *
	 * @since 4.2.0
	 *
	 ...
	 */
	return apply_filters( 'ldgr_filter_get_groups_dashboard_page', $groups_dashboard_pages[0], $groups_dashboard_pages );
}
