...
if ( ! function_exists( 'ir_get_settings' ) ) {
	...
	function ir_get_settings( $key = '', $force = false ) {
		...
		if ( empty( $ir_settings ) ) {
			$ir_settings = get_option( $settings_option_key, array() );
			wp_cache_set( $settings_option_key, $ir_settings, 'instructor_role', 86400 );
		}
...
if ( ! function_exists( 'ir_set_settings' ) ) {
	...
	function ir_set_settings( $key, $value ) {
		...
		//if ( ! empty( $ir_settings ) ) {
			...
			// Update database.
			update_option( $settings_option_key, $ir_settings );
