...
class Notices extends Engine\Base
{
    ...
    public function load_notices()
    {
        ...
        if ( \is_array( $this->settings ) && !isset( $this->settings['archive'] ) ) {
            ...
            if ( !\is_null( \get_page_by_path( $this->settings['slug'], OBJECT ) ) ) {
                $page = \get_page_by_path( $this->settings['slug'], OBJECT );
                \wpdesk_wp_notice( \sprintf(
     ...
     * @return void
     */
    public function dismissable( $string, $key, $type )
    {
        \wpdesk_init_wp_notice_ajax_handler();
        \wpdesk_permanent_dismissible_wp_notice( $string, GT_TEXTDOMAIN . $key, $type );
    }
    ...
    public function content_excerpt_empty()
    {
        ...
        \wpdesk_wp_notice( \__( 'The content and the excerpt of this term are both empty, this will generate empty Tooltips!', GT_TEXTDOMAIN ), 'error' );
        return true;
    }
