...
class Terms_List extends Engine\Base
{
    ...
    public function search_string( string $term )
    {
         ...
         */
        return \apply_filters( $this->default_parameters['filter_prefix'] . '_regex', '/(?<![\\w\\—\\-\\.\\/]|=")(' . $caseinsensitive . ')' . $symbols . '(?![^<]*(\\/>' . $span_open . '|<h|<\\/button|<\\/h|<\\/a|<\\/pre|<\\/figcaption' . $span_close . '|\\"))/' . $unicode, $term );
    }
    ...
    public function default_term_parameters( string $term_value )
    {
        ...
        if ( empty($this->parameters['url']) && empty($this->parameters['type']) || 'internal' === $this->parameters['type'] ) {
