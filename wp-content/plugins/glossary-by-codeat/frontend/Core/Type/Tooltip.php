...
class Tooltip extends Engine\Base
{
    ...
    public function html( array $atts )
    {
        ...
        $html['before'] = '<span class="' . $this->default_parameters['css_class_prefix'] . '-tooltip glossary-term-' . $this->atts['term_ID'] . '" tabindex="0">' . '<span class="' . $class . '">';
    ...
    public function generate_photo( int $theid )
    {
        ...
        if ( !empty($this->settings['t_image']) ) {
            ...
            if ( !\in_array( $theme['tooltip_style'], array( 'box', 'line' ), true ) ) {
                $photo = \get_the_post_thumbnail( $theid, \apply_filters( $this->default_parameters['filter_prefix'] . '_tooltip_image', 'thumbnail' ) );
                if ( !empty($photo) ) {
                    return $photo;
                }
                return '';
